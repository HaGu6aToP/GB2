...
Buchberger_result buchberger(const Basis basis, ulong t, const PolynomRing ctx)
{
    ...
    #if __DEBUG_BUCHBERGER

        for(i = 0; i < t-1; i++)
            for (j = i+1; j < t; j++){
                S(S_poly, g_array_index(G, Polynom, i), g_array_index(G, Polynom, j), ctx);
                printf("S(%ld, %ld) = ", i, j);
                fq_nmod_mpoly_print_pretty(S_poly, NULL, ctx);
                printf("\n");
            }
    ...
    while (P->len > 0)
    {
        pp = g_array_index(P, Pair, P->len - 1);
        S(S_poly, g_array_index(G, Polynom, pp.first), g_array_index(G, Polynom, pp.second), ctx);
        g_array_remove_index(P, P->len - 1);

        #if __DEBUG_BUCHBERGER
                // printf("Pair=(%ld, %ld)\n", pp.first, pp.second);
                // printf("f=");
                // fq_nmod_mpoly_print_pretty(g_array_index(G, Polynom, pp.first), NULL, ctx);
                // printf("\ng=");
                // fq_nmod_mpoly_print_pretty(g_array_index(G, Polynom, pp.second), NULL, ctx);
                // printf("\nS(f, g)=");
                // fq_nmod_mpoly_print_pretty(S_poly, NULL, ctx);
                // printf("\n");
        #endif

        if (!fq_nmod_mpoly_is_zero(S_poly, ctx))
        {
            Q = init_empty_basis(G->len, ctx);
            fq_nmod_mpoly_divrem_ideal(Q, mod, S_poly, (Basis)G->data, G->len, ctx);
            free_basis(Q, G->len, ctx);

            #if __DEBUG_BUCHBERGER
                // printf("S mod G=");
                // fq_nmod_mpoly_print_pretty(mod, NULL, ctx);
                // printf("\n");
            #endif

            if (!fq_nmod_mpoly_is_zero(mod, ctx))
            {
                for (i = 0; i < G->len; i++)
                {
                    Pair pair = {i, G->len};
    ...
    fq_nmod_mpoly_clear(mod, ctx);

    Buchberger_result res = {(Basis)G->data, G->len};
...
SPair find_min_v1(GArray *F, GArray *P, PolynomRing ctx)
{
    ...
    if (fq_nmod_mpoly_is_zero(S_poly, ctx) == 0)
        LT(lt_min, S_poly, ctx);
...
void *log_buchberger_task(void *params)
{
    ...
    if (check_cond(data) == 1)
        my_exit(data);
    if (fq_nmod_mpoly_is_zero(data->S_poly, data->ctx) == 1)
        pthread_exit(NULL);
...
Buchberger_result threaded_buchberger(const Basis basis, ulong t, ulong threads_count, PolynomRing ctx)
{
    ...
    while (P->len > 0)
    {
        ...
        for (i = 0; i < threads_count; i++)
        {
            ...
            if (fq_nmod_mpoly_is_zero(data[i].S_poly, ctx) == 1)
                continue;
...
void GMI_v3(GArray *F, GArray *P, const Polynom h, int t, thread_buchberger_data_v2_t *data, int data_len, PolynomRing ctx)
{
    ...
    while (i < P->len)
    {
        ...
        for (j = 0; j < data_len; j++)
        {
            ...
            if (data[j].executed != 1 || fq_nmod_mpoly_is_zero(data[j].S_poly, ctx) != 1)
                continue;
            if (cmpPair(&sp, &(data[j].pPair)) == 0)
            {
                g_array_remove_index(P, i);
...
//     ulong j, zeros;
//     fq_nmod_mpoly_t mod;
//     fq_nmod_mpoly_t ld1, ld2;

//     fq_nmod_mpoly_init(ld1, ctx);
//     fq_nmod_mpoly_init(ld2, ctx);
//     fq_nmod_mpoly_init(mod, ctx);

//     zeros = 0;
//     while(i < *npoly - 1){
//         if (fq_nmod_mpoly_is_zero(basis[i], ctx) != 1)
//             LT(ld1, basis[i], ctx);
//             for(j = i + 1; j < *npoly; j++){
//                 if (fq_nmod_mpoly_is_zero(basis[j], ctx) == 1) continue;
//                 LT(ld2, basis[j], ctx);
//                 if (fq_nmod_mpoly_divides(mod, ld1, ld2, ctx) == 1){

//                     zeros++;
//                     fq_nmod_mpoly_zero(basis[i], ctx);
//                     break;
//                 }
//             }
...
//     fq_nmod_mpoly_clear(mod, ctx);
//     fq_nmod_mpoly_clear(ld1, ctx);
//     fq_nmod_mpoly_clear(ld2, ctx);

//     *npoly = *npoly - zeros;

//     for(i = 0; i < *npoly; ++i){
//         if (fq_nmod_mpoly_is_zero(basis[i], ctx) != 1) continue;

//         for(j = i+1; j < *npoly+zeros; j++){
//             if (fq_nmod_mpoly_is_zero(basis[j], ctx) == 0){
//                 fq_nmod_mpoly_swap(basis[i], basis[j], ctx);
//                 break;
//             }
//         }
//     }

//     // Basis res = init_empty_basis(npoly - zeros, ctx);

//     // for(i = 0; i < npoly; i++){
//     //     if (fq_nmod_mpoly_is_zero(basis[i], ctx) != 1)
//     //         fq_nmod_mpoly_set(res[i], basis[i], ctx);
//     // }

//     // free_basis(basis, npoly, ctx);
//     // return res;
