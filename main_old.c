...
Polynom* buchberger(Polynom* basis, ulong t, PolynomRing ctx){
    ...
    Pair curr_pair;
    Polynom f, g, mul, lcm, lt_f, lt_g, s, Sp;
    flint_rand_t random;
    ...
    while (B->len != 0){
        ...
        if (!fq_nmod_mpoly_equal(lcm, mul, ctx) && !crit(G, B, i, j, f, g, ctx)){
            Polynom* Q = f_calloc(G->len, sizeof(Polynom));
            S(Sp, f, g, ctx);
            fq_nmod_mpoly_divrem_ideal(Q, s, Sp, (fq_nmod_mpoly)G->data, G->len, ctx);
        }
...
void main(void){
    ...
    // fq_nmod_mpoly_set(NULL, basis[0], poly_ring_ctx);
